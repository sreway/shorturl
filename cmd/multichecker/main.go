...
func main() {
	analyzers := []*analysis.Analyzer{
		...
		nilfunc.Analyzer,
		// inspects the control-flow graph of an SSA function and reports errors such as nil pointer dereferences
		...
		// check HTTP response body is closed and a re-use of TCP connection is not blocked.
		bodyclose.Analyzer,
		// check err of sql.Rows is checked.
		rowserr.NewAnalyzer(
			"github.com/jmoiron/sqlx",
		),
	}

	// analyzes that simplify code.
	for _, v := range simple.Analyzers {
		analyzers = append(analyzers, v.Analyzer)
	}
	// analyzes that find bugs and performance issues.
	for _, v := range staticcheck.Analyzers {
		analyzers = append(analyzers, v.Analyzer)
	}
	// analyzes that enforce style rules.
	for _, v := range stylecheck.Analyzers {
		analyzers = append(analyzers, v.Analyzer)
	}

	// analyzes the presence of the operating os.Exit function in the main package.
